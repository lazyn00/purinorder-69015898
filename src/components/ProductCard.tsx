...
  artist?: string; // <--- ĐÃ THÊM: Trường artist
};

// === HÀM HELPER: ĐỊNH DẠNG NGÀY GIỜ (Giữ nguyên) ===
...
  const deadlineString = product.orderDeadline;
  let isExpired = false;
  let deadlineText = '';

  if (deadlineString) {
    const dateToCheck = new Date(deadlineString.replace(' ', 'T'));
    isExpired = dateToCheck < new Date(); 
    deadlineText = formatDeadline(deadlineString);
  }
...
          />
          ...
          {/* CONTAINER CHO CÁC TAG */}
          <div className="absolute top-1.5 left-1.5 flex flex-col items-start space-y-1">
            {/* TAG STATUS */}
            {product.status && (
              <Badge 
                variant="secondary" 
...
            )}
            ...
            {product.artist && (
              <Badge 
                variant="default" // Có thể dùng 'default' hoặc 'primary' để nổi bật hơn
                className="h-5 px-1.5 text-[10px] bg-purple-500 hover:bg-purple-600"
              >
                {product.artist}
              </Badge>
            )}
...
          {/* HIỂN THỊ HẠN ORDER */}
          {deadlineString && (
            <p className={`mt-1 text-xs font-semibold ${isExpired ? 'text-red-600' : 'text-orange-500'}`}>
              {isExpired ? (
                '❌ Đã hết hạn order'
              ) : (
                `⏰ Hạn Order: ${deadlineText}`
              )}
            </p>
          )}
          
